...
kubeadm init \
    --apiserver-advertise-address=10.1.0.1 \
    --pod-network-cidr=10.244.0.0/16 \
    --image-repository=registry.cn-hangzhou.aliyuncs.com/ates-k8s
```
> 获取token
```shell
kubeadm token list
...
spec:
  template:
    ...
    spec:
      ...
      initContainers:
      - name: install-cni
        image: registry.cn-hangzhou.aliyuncs.com/ates-k8s/flannel:v0.11.0-amd64
        command:
        - cp
        args:
        - -f
...
spec:
  template:
    ...
    spec:
      ...
      initContainers:
      - name: install-cni
        image: registry.cn-hangzhou.aliyuncs.com/ates-k8s/flannel:v0.11.0-arm64
        command:
        - cp
        args:
        - -f
...
spec:
  template:
    ...
    spec:
      ...
      initContainers:
      - name: install-cni
        image: registry.cn-hangzhou.aliyuncs.com/ates-k8s/flannel:v0.11.0-arm
        command:
        - cp
        args:
        - -f
...
spec:
  template:
    ...
    spec:
      ...
      initContainers:
      - name: install-cni
        image: registry.cn-hangzhou.aliyuncs.com/ates-k8s/flannel:v0.11.0-ppc64le
        command:
        - cp
        args:
        - -f
...
spec:
  template:
    ...
    spec:
      ...
      initContainers:
      - name: install-cni
        image: registry.cn-hangzhou.aliyuncs.com/ates-k8s/flannel:v0.11.0-s390x
        command:
        - cp
        args:
        - -f
...
&& rm -f ./kube-flannel.yaml
```

## 加入集群
```shell
kubeadm join 10.1.0.1:6443 --token <TOKEN> \
...
&& docker pull registry.cn-hangzhou.aliyuncs.com/ates-k8s/kubernetes-dashboard-amd64:v1.10.1 \
&& docker tag registry.cn-hangzhou.aliyuncs.com/ates-k8s/kubernetes-dashboard-amd64:v1.10.1 k8s.gcr.io/kubernetes-dashboard-amd64:v1.10.1
```